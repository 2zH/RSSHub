...
module.exports = async (ctx) => {
    ...
    const items = await Promise.all(
        list.map(async (item) => {
            ...
            $$$('.indienova-single-post img.fr-fic.fr-dib').each(function () {
                const $img = $$$(this);
                const src = $img.attr('src');
                const originalImg = src.endsWith('_webp') ? src : src + '_webp';
                $img.attr('src', process.env.IMAGE_RELAY + originalImg);
            })
